...
After every discrete process or major work step in a Conta MCP workflow, display a compact cumulative evidence-weighted progress bar with a percentage and short status label.
...
The exact standalone `Status` command remains the trigger for the expanded Conta MCP status block; it is no longer the exclusive trigger for displaying progress.

The expanded response must contain:

1. the applicable Conta MCP completion target;
...
1. stable read-only production MCP;
2. controlled invoice-draft create capability;
3. controlled edit capability;
4. low-risk production write rollout;
...
Progress never grants deployment, provider-call, mutation or write-enablement authority. Existing explicit operator gates remain mandatory.
